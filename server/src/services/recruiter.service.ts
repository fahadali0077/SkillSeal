...
export async function searchCandidates(recruiterId: string, params: Record<string, unknown>) {
  ...
  const candidates = users.map(u => {
    ...
    return {
      ...
      behaviorIntegrity: integrity(uv[0]?.behaviorScore ?? 100), aiFlag: aiMax > 0.40, connectionDegree: 'none' as const
    };
  });
...
export async function upsertPipeline(recruiterId: string, input: { candidateId: string; jobId?: string; status: string; note?: string }) {
  ...
  const app = await Application.findOneAndUpdate(filter, { $set: { status, ...(note !== undefined ? { recruiterNote: note } : {}) }, $setOnInsert: { candidateId: new Types.ObjectId(candidateId), recruiterId: new Types.ObjectId(recruiterId), ...(jobId ? { jobId: new Types.ObjectId(jobId) } : {}), appliedAt: new Date(), companyId: new Types.ObjectId('000000000000000000000000') } }, { upsert: true, new: true });
