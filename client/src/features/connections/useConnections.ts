...
export function useSuggestions(){return useQuery({queryKey:connKeys.suggestions(),queryFn:()=>apiFetch<{userId:string;firstName:string;lastName:string;profilePhoto:string;headline:string;customUrl:string;mutualConnections:number}[]>(`${API_ORIGIN}/api/v1/suggestions/people`),staleTime:5*60_000});}
