...
import {
  MapPin, ShieldCheck, ShieldAlert, ShieldX,
  Info, MessageSquare, UserPlus, CheckCircle2,
} from 'lucide-react';
...
const TIER_COLORS: Record<string, string> = {
  ...
  intermediate: 'bg-blue-50 text-brand',
  advanced:     'bg-purple-50 text-purple-700',
  expert:       'bg-amber-50 text-amber-700',
};

const DEGREE_LABELS: Record<string, string> = {
  '1st': 'bg-green-50 text-green-700 border-green-200',
  '2nd': 'bg-blue-50 text-brand border-blue-200',
...
function IntegrityBadge({ level }: { level: IntegrityLevel }) {
  const cfg = {
    green:  { icon: <ShieldCheck size={13} />, label: 'High integrity',  cls: 'text-green-600 bg-green-50 border-green-200' },
    yellow: { icon: <ShieldAlert size={13} />, label: 'Some concerns',   cls: 'text-amber-600 bg-amber-50 border-amber-200' },
    red:    { icon: <ShieldX     size={13} />, label: 'Integrity flag',  cls: 'text-red-600 bg-red-50 border-red-200' },
  }[level];
...
function AIFlag() {
  ...
  return (
    ...
    >
      <button className="text-amber-500 hover:text-amber-600 p-0.5">
        <Info size={14} />
      </button>
      {show && (
        <motion.div
          ...
          className="absolute bottom-full right-0 mb-1 bg-gray-900 text-white text-xs rounded-xl px-3 py-2 w-48 z-20"
        >
          AI-assisted response probability &gt;40%.
        </motion.div>
      )}
    </div>
  );
...
export default function CandidateCard({
  ...
  const pipeline = useUpsertPipeline();
  const [saved, setSaved] = useState(false);

  const handleSave = async () => {
    await pipeline.mutateAsync({ candidateId: candidate.userId, status: 'shortlisted' });
    setSaved(true);
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <Link to={`/recruiter/candidates/${candidate.userId}`} className="shrink-0">
          {candidate.profilePhoto
            ? <img src={candidate.profilePhoto} alt={candidate.fullName} className="w-12 h-12 rounded-full object-cover" />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <Link
              to={`/recruiter/candidates/${candidate.userId}`}
        ...
        <div className="flex flex-wrap gap-1.5">
          {candidate.verifiedSkills.map(skill => (
            ...
            >
              ...
              <span className={`px-1 rounded text-[10px] font-semibold capitalize ${TIER_COLORS[skill.tier] ?? ''}`}>
                {skill.tier}
              </span>
              <span className="text-gray-500">{skill.compositeScore}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <div className="flex gap-1.5">
          <Link
            to={`/messages?new=${candidate.userId}`}
          ...
          <Link
            to={`/recruiter/candidates/${candidate.userId}`}
            className="text-xs font-medium text-brand hover:text-brand-dark px-2.5 py-1.5 rounded-lg hover:bg-blue-50"
          >
            View
          </Link>
          <button
            onClick={handleSave}
            disabled={pipeline.isPending || saved}
            className={`flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-lg transition-colors ${
              saved ? 'bg-green-50 text-green-700' : 'bg-brand text-white hover:bg-brand-dark'
            }`}
          >
            {saved
