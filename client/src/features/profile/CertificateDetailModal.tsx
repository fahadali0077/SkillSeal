...
export default function CertificateDetailModal({skill,isOwner,onClose}:Props){
  ...
  return(<AnimatePresence><motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={e=>e.target===e.currentTarget&&onClose()}>
    <motion.div initial={{y:24,opacity:0,scale:0.97}} animate={{y:0,opacity:1,scale:1}} exit={{y:24,opacity:0,scale:0.97}} transition={{type:'spring',damping:24,stiffness:300}} className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
      <div className={`bg-ink-900 p-6 relative shrink-0`}>
        <button onClick={onClose} className="absolute top-4 right-4 p-1.5 rounded-lg bg-ink-800 hover:bg-ink-700 text-paper"><X size={16}/></button>
        <div className="flex items-center gap-3"><div className="w-14 h-14 rounded bg-ink-800 flex items-center justify-center shrink-0"><ShieldCheck size={26} className="text-paper"/></div><div className="min-w-0"><p className="label text-ink-300">Verified Certificate</p><h2 className="font-display text-2xl text-paper break-words">{skill.skillName}</h2><p className="text-sm text-ink-300 capitalize">{TIER_LABELS[skill.tier]??skill.tier} Level</p></div></div>
      </div>
