...
export default function CertificateDetailModal({skill,isOwner,onClose}:Props){
  ...
  return(<AnimatePresence><motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={e=>e.target===e.currentTarget&&onClose()}>
    <motion.div initial={{y:24,opacity:0,scale:0.97}} animate={{y:0,opacity:1,scale:1}} exit={{y:24,opacity:0,scale:0.97}} transition={{type:'spring',damping:24,stiffness:300}} className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
      <div className={`bg-gradient-to-r ${TIER_COLORS[skill.tier]??TIER_COLORS.beginner} p-6 relative`}>
        ...
        <div className="flex items-center gap-3"><div className="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center"><ShieldCheck size={28} className="text-white"/></div><div><p className="text-white/70 text-xs font-semibold uppercase tracking-wider">Verified Certificate</p><h2 className="text-white text-xl font-bold">{skill.skillName}</h2><p className="text-white/80 text-sm capitalize">{TIER_LABELS[skill.tier]??skill.tier} Level</p></div></div>
      </div>
