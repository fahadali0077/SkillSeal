...
export default function UpgradePrompt({feature,description,requiredPlan='pro',variant='card',className=''}:Props){
  ...
  if(variant==='banner')return(<motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} className={`flex items-center justify-between gap-4 px-5 py-3 bg-ink-900 text-paper rounded ${className}`}><div className="flex items-center gap-3 min-w-0"><Zap size={18} className="shrink-0"/><p className="text-sm font-medium truncate"><span className="font-bold">{feature}</span> is available on {label}. {description}</p></div><Link to="/pricing" className="shrink-0 text-xs font-semibold bg-seal-600 text-paper px-3 py-1.5 rounded hover:bg-seal-700 flex items-center gap-1">Upgrade<ArrowRight size={12}/></Link></motion.div>);
