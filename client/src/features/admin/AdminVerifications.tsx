  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <p className="text-xs text-gray-400">Revoking invalidates a certificate and removes it from recruiter search.</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-gray-500 uppercase tracking-wide bg-gray-50/70 border-b border-gray-100">
                ...
                <th className="px-4 py-3 font-semibold hidden md:table-cell">AI risk</th>
                <th className="px-4 py-3 font-semibold">Status</th>
                <th className="px-4 py-3 font-semibold hidden lg:table-cell">Issued</th>
                <th className="px-4 py-3 font-semibold text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {isLoading && Array.from({ length: 8 }).map((_, i) => (
                <tr key={i}><td colSpan={7} className="px-4 py-3"><div className="h-8 skeleton rounded-lg" /></td></tr>
              ))}
              {isError && <tr><td colSpan={7} className="px-4 py-10 text-center text-red-600">{(error as Error)?.message ?? 'Failed to load.'}</td></tr>}
                  ...
                  <tr key={v._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        {v.userId && <a href={`/profile/${v.userId}`} target="_blank" rel="noreferrer" className="text-gray-300 hover:text-brand"><ExternalLink size={13} /></a>}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-700">{v.skillName} <span className="text-gray-400 text-xs">· {v.tier}</span></td>
                    <td className="px-4 py-3 font-semibold text-gray-900">{v.compositeScore}%</td>
                    ...
                    <td className="px-4 py-3"><VerifBadge status={v.status} /></td>
                    <td className="px-4 py-3 text-gray-500 hidden lg:table-cell">{new Date(v.issuedAt).toLocaleDateString()}</td>
        ...
        {revokeTarget && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[92%] max-w-md bg-white rounded-2xl shadow-2xl p-6">
              ...
              <p className="text-sm text-gray-600 mb-4">Revoking <span className="font-semibold">{revokeTarget.skill}</span> for {revokeTarget.name}. The reason is stored on the record for audit.</p>
