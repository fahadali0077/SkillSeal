...
const SKILLS = [{ id: 'react-skill-id', slug: 'react', name: 'React', category: 'Frontend', icon: '⚛️' }, { id: 'nodejs-skill-id', slug: 'nodejs', name: 'Node.js', category: 'Backend', icon: '🟢' }, { id: 'mongodb-skill-id', slug: 'mongodb', name: 'MongoDB', category: 'Database', icon: '🍃' }];
...
export default function AssessmentLanding() {
  ...
  const prefilledSkill = SKILLS.find(s => s.id === preSkillId) ?? null;
  const [step, setStep] = useState<'skill' | 'tier' | 'preflight'>(prefilledSkill && preTier ? 'preflight' : prefilledSkill ? 'tier' : 'skill');
  const [selectedSkill, setSkill] = useState(prefilledSkill); const [selectedTier, setTier] = useState<SkillTier | null>(preTier);
  ...
  const handleStart = async () => { if (!selectedSkill || !selectedTier || !agreed) return; await startSession(selectedSkill.id, selectedSkill.name, selectedTier); navigate('/assessment/active'); };
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <AnimatePresence mode="wait">
        {step === 'skill' && (<motion.div key="skill" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
          ...
          <div className="grid gap-3">{SKILLS.map(skill => (<button key={skill.id} onClick={() => { setSkill(skill); setStep('tier'); }} className={`flex items-center gap-4 p-4 rounded-xl border text-left transition-all ${selectedSkill?.id === skill.id ? 'border-brand bg-blue-50' : 'border-gray-200 hover:border-brand/50'}`}><span className="text-3xl">{skill.icon}</span><div className="flex-1"><p className="font-semibold text-gray-900">{skill.name}</p><p className="text-sm text-gray-500">{skill.category}</p></div><ChevronRight size={16} className="text-gray-300" /></button>))}</div>
        </motion.div>)}
