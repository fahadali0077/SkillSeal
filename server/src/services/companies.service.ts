...
export async function createCompany(adminId:string,input:Record<string,unknown>){
  ...
  const doc=await Company.create({name:input.name,slug,...input,adminUsers:[new Types.ObjectId(adminId)],followerCount:0});
  return ser(doc);
}
...
export async function followCompany(slug:string,userId:string):Promise<void>{
  ...
  await Company.findByIdAndUpdate(c._id,{$inc:{followerCount:1}});
}
...
export async function listCompanies(page = 1, limit = 20) {
  ...
  const [companies, total] = await Promise.all([
    Company.find({ isActive: true })
      .select('name slug logo tagline industry followerCount employeeCount')
