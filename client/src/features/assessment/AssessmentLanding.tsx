...
import { ShieldCheck, ChevronRight, Clock, AlertTriangle, CheckCircle2, Monitor, Wifi, Eye, Loader2, ArrowLeft } from 'lucide-react';
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'tier' && (<motion.div key="tier" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
          <div className="flex items-center gap-3 mb-4"><button onClick={() => setStep('skill')} className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition-colors"><ArrowLeft size={14} />Back</button><h2 className="font-semibold text-gray-900">Select tier for <span className="text-brand">{selectedSkill?.name}</span></h2></div>
        ...
        {step === 'preflight' && selectedSkill && selectedTier && (<motion.div key="preflight" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
          <div className="flex items-center gap-3 mb-2"><button onClick={() => setStep('tier')} className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition-colors"><ArrowLeft size={14} />Back</button><h2 className="font-semibold text-gray-900">Review before starting</h2></div>
